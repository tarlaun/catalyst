...
All commands are available through the `catalyst` CLI.

```bash
catalyst --help
```

### `catalyst tile` — Partition a dataset

```bash
catalyst tile --input data.parquet --outdir datasets/mydata --num-tiles 40
```
...
### `catalyst mvt` — Generate vector tiles

```bash
catalyst mvt --dir datasets/mydata --zoom 7 --threshold 100000
```
...
### `catalyst build` — Full pipeline (tile + MVT)

```bash
catalyst build --input data.parquet --outdir datasets/mydata
```

### `catalyst serve` — Launch the tile server

```bash
catalyst serve --dir datasets --port 8765
```
...
### `catalyst info` — Inspect a dataset

```bash
catalyst info --dir datasets/mydata
```
...
See [`catalyst/_internal/server/llm/README.md`](catalyst/_internal/server/llm/README.md) for full LLM provider documentation.

## Example

```bash
# Full pipeline
catalyst build --input ../data/TIGER2018_COUNTY.parquet --outdir datasets/TIGER2018_COUNTY
