...
def tile(
    ...
    *,
    ...
    from catalyst._internal.tiling.datasource import GeoParquetSource, GeoJSONSource, is_geojson_path
    from catalyst._internal.tiling.assigner import TileAssignerFromCSV, RSGroveAssigner
    ...
    build_histograms_for_dir(
        tiles_dir=tiles_dir,
        # Tiles are written with a canonical 'geometry' column (WriterPool
        # normalizes it), so histograms always read 'geometry'.
        outdir=hist_dir,
        geom_col="geometry",
        grid_size=4096,
        dtype="float64",
...
def build(
    ...
    **tile_kwargs
    ...
    pmtiles_path = None
    if pmtiles:
        from catalyst._internal.pmtiles.exporter import export_to_pmtiles
