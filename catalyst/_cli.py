"""Click CLI for catalyst: spatial tiling, MVT generation, and tile serving."""
...
@click.group()
@click.version_option(package_name="catalyst")
def main():
    """catalyst — spatial tiling, MVT generation, and tile serving."""
...
def serve(data_dir, host, port, cache_size, log_level):
    ...
    app = catalyst.create_app(data_dir=data_dir, cache_size=cache_size)
    click.echo(f"Starting catalyst server on {host}:{port}")
